#!/bin/bash
# MCDManager官方安装脚本.
# 这个脚本将会把MCDManager服务端和节点服务端更新/安装至最新发布版本.
# ------------------------------------------------------------------------------
# 受支持的Linux:
# 此脚本支持以下Linux发行版:
...
# Node.js installation path (defaults to the MCDManager installation path. Can be overridden with --node-install-dir)
...
# Function to download MCDM package. fetch from primary URL first, then fallback URL.
...
download_mcsm() {
  ...
  cprint cyan bold "下载MCDManager安装包…"
  ...
  local extracted_tmp_path="${tmp_dir}/mcdm_${suffix}"
  ...
  install_tmp_dir="${install_dir}/mcdm_${suffix}"
  ...
  cprint green bold "MCDManager源提取并移动到: $install_tmp_dir"
  return 0
}
...
# Function to stop MCDM services if they exist
stop_mcsm_services() {
  cprint yellow bold "试图停止mcdm-web和mcdm-daemon服务..."

  # Attempt to stop mcdm-web
  cprint blue "正在停止 mcdm-web..."
  if systemctl stop mcdm-web; then
    cprint green "mcdm-web 已停止."
  else
    cprint red bold "警告：未能停止mcdm-web（可能不存在或已停止）."
  fi

  # Attempt to stop mcdm-daemon
  cprint blue "正在停止 mcdm-daemon..."
  if systemctl stop mcdm-daemon; then
    cprint green "mcdm-daemon 已停止."
  else
    cprint red bold "警告：未能停止mcdm-daemon（可能不存在或已停止）."
  fi
}
...
[Unit]
Description=MCDManager-${component^}
...
extract_component_info() {
  ...
  if [[ "$install_daemon" == true ]]; then
    local daemon_service="mcdm-daemon.service"
  ...
  if [[ "$install_web" == true ]]; then
    local web_service="mcdm-web.service"
...
print_install_result() {
  ...
  cprint yellow noprefix "MCDManager管理命令:"
  if [[ "$install_daemon" == true ]]; then
    cprint white noprefix nonl "  systemctl start   "
	cprint yellow noprefix "mcdm-daemon.service"
  ...
  if [[ "$install_web" == true ]]; then
    cprint white noprefix nonl "  systemctl start   "
	cprint yellow noprefix "mcdm-web.service"
    cprint white noprefix nonl "  systemctl stop    "
    cprint yellow noprefix "mcdm-web.service"
    cprint white noprefix nonl "  systemctl restart "
    cprint yellow noprefix "mcdm-web.service"
  ...
  # Closing message
  cprint green noprefix  "安装完成。享受使用MCDManager管理服务器的乐趣!"
  echo ""
}
...
main() {
  ...
  safe_run download_mcsm "获取MCDManager源失败。处理步骤"
  safe_run mcsm_install_prepare "准备安装时出错"
  
  safe_run install_mcsm "未能安装 MCDManager"
}
main "$@"